...
**Tests**: Tests are REQUIRED for behavior changes and bug fixes. Write the failing
test or executable acceptance check before implementation. Documentation-only or
purely mechanical changes may omit new tests only when tasks record why behavior is
unchanged.
...
- [ ] T010 Define authoritative inputs, managed outputs, and manifest ownership boundaries
- [ ] T011 [P] Add adapter contract fixtures for affected agents and platforms
- [ ] T012 Add safe-write, drift-detection, and failure-recovery test infrastructure
...
- [ ] TXXX [P] Add targeted unit tests for remaining behavior risks in tests/unit/
- [ ] TXXX Security hardening
- [ ] TXXX Verify repeat synchronization produces no additional diff
- [ ] TXXX Verify interrupted/invalid input leaves managed and unmanaged files recoverable
- [ ] TXXX Run affected adapter contract tests across declared platforms/sync modes
- [ ] TXXX Run `agent21 doctor` and capture a clean or explicitly justified report
